...
public class MZDepositoMedioPago extends X_Z_DepositoMedioPago implements DocAction, DocOptions {
    ...
    public MZDepositoMedioPago (Properties ctx, int Z_DepositoMedioPago_ID, String trxName)
    {
      super (ctx, Z_DepositoMedioPago_ID, trxName);
    }

    /** Load Constructor */
    public MZDepositoMedioPago (Properties ctx, ResultSet rs, String trxName)
    {
    ...
    {
      StringBuffer sb = new StringBuffer ("MZDepositoMedioPago[")
	...
	public void updateTotals(){
		...
		try{
			// Obtengo suma de montos de medios de pago a depositar
			sql = " select sum(coalesce(totalamt,0)) as total from z_depositompagolin " +
					" where z_depositomediopago_id =" + this.get_ID();
			BigDecimal sumMedios = DB.getSQLValueBDEx(get_TrxName(), sql);
			if (sumMedios == null) sumMedios = Env.ZERO;

			action = " update z_depositomediopago set totalamt =" + sumMedios +
					 " where z_depositomediopago_id =" + this.get_ID();
			DB.executeUpdateEx(action, get_TrxName());
		}
		catch (Exception e){
